...
import { moveCart } from '../store/feature/cartSlice';
...
const CartsMenu = () => { 
    ...
    const { carts, cartsSeq } = useSelector(store => store.cart);
        ...
        return (
            <span className='flex-center cursor-default text-sm justify-between mr-24 mb-1'>
                ...
                <span className='flex-center'>
                    ...
                    <span className={`${cl} lg:block hidden`}>Last Modified</span>
                    <span className={`${cl} block w-auto`}>Size&nbsp;</span>
                </span>
            </span>
        )
    }
    return (
        <animated.div className='flex flex-col items-stretch h-full grow'>
            <Header>
                <Link to='/' className={`flex-center w-full ${!fs ? `border-b-[2px] pb-4` : ``}`}>
            ...
            <DroppablerDiv dispatchFunc={moveCart} isDropDisabled={!editingMode}>

                {!cartsSeq ? `You have no carts.` :
                    <>
                        {fs && <HeadingRow />}

                        {cartsSeq.map((_id, i) => (
    
                            <CartItem key={_id} cart={carts[_id]} index={i} dndDisabled={!editingMode} fs={fs} setFs={setFs}/>
